...
class DataTransformation:
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            logging.info('Replacing "?" values with NaN')
            train_df.replace('?', np.nan, inplace=True)
            test_df.replace('?', np.nan, inplace=True)
            logging.info('Replacement completed')

            logging.info('Finding the number of null values in each column')
            null_values_count = train_df.isnull().sum()
            logging.info(f'Null values count:\n{null_values_count}')

            logging.info('Unique values in the "workclass" column:')
            logging.info(train_df['workclass'].unique())
            ...
            ada = ADASYN()
            input_feature_train_arr, target_feature_train_df = ada.fit_resample(
